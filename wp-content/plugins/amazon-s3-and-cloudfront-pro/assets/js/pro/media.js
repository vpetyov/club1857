(function( $, _ ) {
	...
	media.view.AttachmentsBrowser = wpAttachmentsBrowser.extend( {
		createToolbar: function() {
			...
			// Basic props we'll use for controls regardless of WP version.
			var controlProps = {
				controller: this.controller,
			...
			var OffloadLocationFilterContainer = media.view.OffloadLocationFilter;
			var OffloadAccessFilterContainer = media.view.OffloadAccessFilter;

			if ( as3cfpro_media.filters.use_containers ) {
				var offloadLocationFilterLabel = new wp.media.view.Label( {
					value: as3cfpro_media.filters.as3cf_location.label,
					attributes: {
						"for": "media-attachment-as3cf-location-filter"
					},
					priority: -75
				} );

				var offloadLocationFilter = new media.view.OffloadLocationFilter( controlProps );

				OffloadLocationFilterContainer = wp.media.View.extend( {
					tagname: "div",
					className: "media-filter-container as3cf-location-filter",

					initialize: function() {
						this.views.add( [offloadLocationFilterLabel, offloadLocationFilter] );
					}
				} );

				var offloadAccessFilterLabel = new wp.media.view.Label( {
					value: as3cfpro_media.filters.as3cf_access.label,
					attributes: {
						"for": "media-attachment-as3cf-access-filter"
					},
					priority: -75
				} );

				var offloadAccessFilter = new media.view.OffloadAccessFilter( controlProps );

				OffloadAccessFilterContainer = wp.media.View.extend( {
					tagname: "div",
					className: "media-filter-container as3cf-access-filter",

					initialize: function() {
						this.views.add( [offloadAccessFilterLabel, offloadAccessFilter] );
					}
				} );
			}

			// Add the locations filter
			this.toolbar.set( "offloadLocationFilter", new OffloadLocationFilterContainer( controlProps ) );

			// Add the access filter
			this.toolbar.set( "offloadAccessFilter", new OffloadAccessFilterContainer( controlProps ) );
