<script>
	...
	let isRunning = $derived( !!$running );
	let tool = $derived( runningTool( $tools, $running ) );
	let icon = $derived( tools.icon( tool, isRunning, true ) );
	...
	let disabled = $derived( isRunning && (($running && $running !== tool.id) || (tool.is_processing && tool.is_paused) || tool.is_cancelled || $toolsLocked) );

	let starting = $derived( !!(isRunning && tool.progress < 1 && !tool.is_paused) );
	let status = $derived( isRunning ? "(" + numToShortString( tool.queue.processed ) + "/" + numToShortString( tool.queue.total ) + ") " + toolStatus( tool, isRunning ) : "" );
	let title = $derived( isRunning ? tool.name + ": " + tool.progress + "% (" + numToString( tool.queue.processed ) + "/" + numToString( tool.queue.total ) + ")" : "" );
	...
	let percentComplete = $derived( getPercentComplete( tool, isRunning ) );
</script>
...
{#if tool}
	<div class="nav-status-wrapper tool-running">
		...
		<div class="nav-status" {title} onclick={() => push("/tools")}>
