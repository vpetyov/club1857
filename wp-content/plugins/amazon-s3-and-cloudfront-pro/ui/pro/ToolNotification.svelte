<script>
	...
	let { notification, children } = $props();
					...
					{#each notification.errors.details as item, index}
						<div class="item" transition:fade>
							<div class="summary">
								...
								<button class="dismiss" onclick={(event) => {event.preventDefault(); dismissAll(notification.errors.tool_key, item);}}>{$strings.dismiss}</button>
							</div>
							<ul class="detail">
