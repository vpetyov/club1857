<script>
	...
	let { tool = {}, status_right } = $props();

	// Total processed related variables.
	let showTotal = $derived( !!tool.hasOwnProperty( "total_progress" ) );
	let initial = $derived( !!(showTotal && tool.total_progress < 1) );
	let partialComplete = $derived( !!(showTotal && tool.total_progress > 0 && tool.total_progress < 100) );
	let complete = $derived( !!(showTotal && !initial && !partialComplete) );

	// In progress related variables.
	let isRunning = $derived( !!($running && $running === tool.id) );
	let starting = $derived( !!(isRunning && tool.progress < 1 && !tool.is_paused) );
	...
	let disabled = $derived( ($running && $running !== tool.id) || (tool.is_processing && tool.is_paused) || tool.is_cancelled || $toolsLocked );
	let disabled_bucket_access = $derived( tool.requires_bucket_access && !$bucket_writable );
	...
	let percentComplete = $derived( getPercentComplete( tool, isRunning, showTotal ) );
	...
	let icon = $derived( getIcon( tool, isRunning ) );
	...
	let relatedTools = $derived( getRelatedTools( tool ) );
	...
	<PanelRow header>
		...
		<div class="buttons-right">
			...
			{:else}
				...
				{:else if initial}
					<Button primary {disabled} title={disabled ? $strings.disabled_tool_button : ""} onclick={handleStart}>{@html tool.button}</Button>
				{:else if partialComplete}
					<Button primary {disabled} title={disabled ? $strings.disabled_tool_button : ""} onclick={handleStart}>{@html tool.button_partial_complete}</Button>
				{:else}
					<Button primary {disabled} title={disabled ? $strings.disabled_tool_button : ""} onclick={handleStart}>{@html tool.button}</Button>
				{/if}
			{/if}
			...
			<PanelRow class="body flex-column" footer>
				{#each [...relatedTools] as [key, relatedTool] }
					<p>
						<a
							href={$urls.settings}
							onclick={(event) => {event.preventDefault(); handleStartTool(relatedTool);}}
