...
class Media_Library_Pro extends Media_Library {
	...
	public function media_action_strings( $strings ) {
		...
		$strings['local_warning']      = __(
			'This file does not exist locally so removing it from the bucket will result in broken links on your site. Are you sure you want to continue?',
			'amazon-s3-and-cloudfront'
		);
		$strings['bulk_local_warning'] = __(
			'Some files do not exist locally so removing them from the bucket will result in broken links on your site. Are you sure you want to continue?',
			'amazon-s3-and-cloudfront'
		);
		...
		$strings['change_acl_error']   = __(
			'There was an error changing the ACL. Make sure the IAM user has permission to change the ACL and try again.',
			'amazon-s3-and-cloudfront'
		);
	...
	public function add_media_row_actions( array $actions, $post ) {
		...
		if (
			in_array( 'copy', $available_actions ) &&
			$file_exists &&
			( ! $as3cf_item || $as3cf_item->served_by_provider( true ) )
		) {
		...
		if ( in_array( 'private_acl', $available_actions ) && ! $as3cf_item->is_private() ) {
			if ( 'grid' === $this->render_context ) {
				...
				$this->add_media_row_action(
					$this->access_col_actions,
					$post_id,
					'private_acl',
					__( 'Make Private', 'amazon-s3-and-cloudfront' )
				);
			}
		}
		...
		if ( in_array( 'public_acl', $available_actions ) && $as3cf_item->is_private() ) {
			if ( 'grid' === $this->render_context ) {
				...
				$this->add_media_row_action(
					...
					$post_id,
					'public_acl',
					__( 'Make Public', 'amazon-s3-and-cloudfront' )
				);
			}
		}
		...
		if ( is_null( $this->messages ) ) {
			$this->messages = array(
				'copy'         => array(
					...
					'error'   => __(
						'There were errors when copying the media to bucket.',
						'amazon-s3-and-cloudfront'
					),
				),
				'remove'       => array(
					...
					'error'   => __(
						'There were errors when removing the media from bucket.',
						'amazon-s3-and-cloudfront'
					),
				),
				'download'     => array(
					...
					'error'   => __(
						'There were errors when downloading the media from bucket.',
						'amazon-s3-and-cloudfront'
					),
				),
				'private_acl'  => array(
					...
					'error'   => __(
						'There were errors when setting the media as private in bucket.',
						'amazon-s3-and-cloudfront'
					),
				),
				'public_acl'   => array(
					...
					'error'   => __(
						'There were errors when setting the media as public in bucket.',
						'amazon-s3-and-cloudfront'
					),
				),
				'remove_local' => array(
					...
					'error'   => __(
						'There were errors when removing the media from server.',
						'amazon-s3-and-cloudfront'
					),
				),
			);
	...
	public function get_acl_value_string( $acl, $post_id ) {
		$as3cf_item = Media_Library_Item::get_by_source_id( $post_id );
		if (
			! in_array( 'update_acl', $this->get_available_media_actions( 'singular' ) ) ||
			! $this->as3cf ||
			! $as3cf_item->served_by_provider( true )
		) {
			return parent::get_acl_value_string( $acl, $post_id );
		}

		return sprintf(
			'<a id="as3cfpro-toggle-acl" title="%s" data-currentACL="%s" href="#">%s</a>',
			$acl['title'],
			$acl['acl'],
			$acl['name']
		);
	...
	public function media_library_filter_query( WP_Query $query ): WP_Query {
		...
		if (
			'all' === $location &&
			'all' === $access &&
			! in_array( $orderby, array_keys( $this->managed_columns ) )
		) {
			return $query;
		}
	...
	private function get_media_library_filters_config(): array {
		...
		$this->media_library_filters['as3cf_location'] = array(
			'id'       => 'as3cf_location',
			'name'     => 'as3cf_location',
			'label'    => __( 'Filter by location', 'amazon-s3-and-cloudfront' ),
		...
		$this->media_library_filters['as3cf_access'] = array(
			'id'       => 'as3cf_access',
			'name'     => 'as3cf_access',
			'label'    => __( 'Filter by access level', 'amazon-s3-and-cloudfront' ),
		...
		$this->media_library_filters['use_containers'] = is_wp_version_compatible( '7.0' );
	...
	public function add_urls_to_summary_counts( array $summaries ): array {
		...
		foreach ( $summaries as $idx => $summary ) {
			if ( ! empty( $summary['type'] ) && Media_Library_Item::summary_type() === $summary['type'] ) {
				$summaries[ $idx ]['offloaded_url']     = add_query_arg(
					'as3cf_location',
					'offloaded',
					$attachments_url
				);
				$summaries[ $idx ]['not_offloaded_url'] = add_query_arg(
					'as3cf_location',
					'not_offloaded',
					$attachments_url
				);
