...
class AS3CF_Pro_Plugin_Compatibility extends AS3CF_Plugin_Compatibility {
	 ...
	 * as we have used the stream wrapper to do any uploading to S3.
	 *
	 * @param bool  $pre
	 * @param array $data
	 * @param int   $post_id
	 ...
	 * @return bool
	 */
	public function abort_update_attachment_metadata( $pre, $data, $post_id ) {
		...
		foreach ( $callers as $caller ) {
			if (
				...
				in_array( $caller['function'], $this->plugin_functions_abort_upload )
			) {
				if (
					$this->as3cf->get_setting( 'remove-local-file' ) ||
					! file_exists( get_attached_file( $post_id, true ) )
				) {
