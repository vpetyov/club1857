...
namespace Pressidium\WP\CookieConsent\Dependencies\GuzzleHttp\Handler;

use Pressidium\WP\CookieConsent\Dependencies\GuzzleHttp\Exception\ConnectException;
use Pressidium\WP\CookieConsent\Dependencies\GuzzleHttp\Exception\RequestException;
use Pressidium\WP\CookieConsent\Dependencies\GuzzleHttp\Promise as P;
use Pressidium\WP\CookieConsent\Dependencies\GuzzleHttp\Promise\FulfilledPromise;
use Pressidium\WP\CookieConsent\Dependencies\GuzzleHttp\Promise\PromiseInterface;
use Pressidium\WP\CookieConsent\Dependencies\GuzzleHttp\Psr7\LazyOpenStream;
use Pressidium\WP\CookieConsent\Dependencies\GuzzleHttp\TransferStats;
use Pressidium\WP\CookieConsent\Dependencies\GuzzleHttp\Utils;
use Pressidium\WP\CookieConsent\Dependencies\Psr\Http\Message\RequestInterface;
use Pressidium\WP\CookieConsent\Dependencies\Psr\Http\Message\UriInterface;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \Pressidium\WP\CookieConsent\Dependencies\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri): string
    {
        ...
        $redactedUriString = \Pressidium\WP\CookieConsent\Dependencies\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \Pressidium\WP\CookieConsent\Dependencies\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \Pressidium\WP\CookieConsent\Dependencies\GuzzleHttp\Psr7\Utils::streamFor($sink);
