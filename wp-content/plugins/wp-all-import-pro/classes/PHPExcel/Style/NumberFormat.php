...
class PHPExcel_Style_NumberFormat extends PHPExcel_Style_Supervisor implements PHPExcel_IComparable
{
    ...
    private static function formatAsFraction(&$value, &$format)
    {
        ...
        $adjustedDecimalPart = empty((float)$GCD) ? (float)$decimalPart :(float)$decimalPart/(float)$GCD;
        $adjustedDecimalDivisor = empty((float)$GCD) ? (float)$decimalDivisor : (float)$decimalDivisor/(float)$GCD;
