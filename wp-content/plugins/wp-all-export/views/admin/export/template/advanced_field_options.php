<?php // phpcs:ignoreFile WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound -- variables in template files inherited from controller render() scope ?>
...
<div class="wp-all-export-advanced-field-options-content">
	...
	<div class="input cc_field sql_field_type" style="margin-left:25px;">
		<a href="#help" rel="sql" class="help" style="display:none;" title="<?php esc_html_e('%%ID%% will be replaced with the ID of the post being exported, example: SELECT meta_value FROM wp_postmeta WHERE post_id=%%ID%% AND meta_key=\'your_meta_key\';', 'wp-all-export'); ?>">?</a>
	...
	<div class="input cc_field repeater_field_type" style="margin-left:25px;">
		...
		<label for="repeater_field_item_per_line"><?php esc_html_e("Display each repeater row in its own csv line", "wp-all-export"); ?></label>
		<div class="input switcher-target-repeater_field_item_per_line" style="margin-top: 10px; padding-left: 15px;">
			...
			<label for="repeater_field_fill_empty_columns"><?php esc_html_e("Fill in empty columns", "wp-all-export"); ?></label>
			<a href="#help" class="wpallexport-help" style="position: relative; top: 0px;" title="<?php esc_html_e('If enabled, each repeater row will appear as its own csv line with all post info filled in for every column.', 'wp-all-export'); ?>">?</a>
		</div>
	</div>
	...
	<div class="input cc_field image_field_type" style="margin-left:25px;">
		<div class="input">
			...
			<label for="is_image_export_featured"><?php esc_html_e("Export featured image", "wp-all-export"); ?></label>
		</div>
		<div class="input">
			...
			<label for="is_image_export_attached_images"><?php esc_html_e("Export attached images", "wp-all-export"); ?></label>
			<div class="switcher-target-is_image_export_attached_images" style="margin: 5px 2px;">
				<label><?php esc_html_e("Separator", "wp-all-export"); ?></label>
	...
	<div class="input cc_field wpae-select-field date_field_type" style="margin-left:25px;">
		<select class="date_field_export_data" style="width: 100%;">
			<option value="unix"><?php esc_html_e("UNIX timestamp - PHP time()", "wp-all-export");?></option>
			<option value="php"><?php esc_html_e("Natural Language PHP date()", "wp-all-export");?></option>
		</select>
		<div class="input pmxe_date_format_wrapper">
			<label style="padding:4px; display: block;"><?php esc_html_e("date() Format", "wp-all-export"); ?></label>
	...
	<div class="input cc_field linked_field_type" style="margin-left:25px;">
		<select class="linked_field_export_data" style="width: 100%; height: 30px;">
			<option value="sku"><?php esc_html_e("Product SKU", "wp-all-export");?></option>
			<option value="id"><?php esc_html_e("Product ID", "wp-all-export");?></option>
			<option value="name"><?php esc_html_e("Product Name", "wp-all-export");?></option>
		</select>				
	</div>
	<!-- PHP snippet options -->
	<div class="input php_snipped" style="margin-top:0;margin-left:24px;">
		...
		<label for="coperate_php"><?php esc_html_e("Export the value returned by a PHP function", "wp-all-export"); ?></label>
		<a href="#help" class="wpallexport-help" title="<?php esc_html_e('The value of the field chosen for export will be passed to the PHP function.', 'wp-all-export'); ?>" style="top: 0;">?</a>
