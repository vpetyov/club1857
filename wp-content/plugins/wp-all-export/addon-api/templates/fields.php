...
<p class="wpae-available-fields-group">
    <?php echo esc_html( $addon->name() ); ?><span class="wpae-expander">+</span>
</p>
                ...
                <li>
                    <div class="default_column" rel="">
                        ...
                        <input type="hidden" name="rules[]"
                               value="pmxe_<?php echo esc_attr( $addon->slug ); ?>_<?php echo esc_attr( ( ! empty( $group['ID'] ) ) ? $group['ID'] : $group['id'] ); ?>"/>
                    </div>
                </li>
                <?php foreach ( $group['fields'] as $field ) { ?>
                    <li class="pmxe_<?php echo esc_attr( $addon->slug ); ?>_<?php echo esc_attr( ( ! empty( $group['ID'] ) ) ? $group['ID'] : $group['id'] ); ?> wp_all_export_auto_generate">
                        <div class="custom_column" rel="<?php echo esc_attr( ( $i + 1 ) ); ?>">
                            <label class="wpallexport-xml-element"><?php echo esc_html( $field['label'] ); ?></label>
                            ...
                            <input type="hidden" name="cc_type[]" value="<?php echo esc_attr( $addon->slug ); ?>"/>
