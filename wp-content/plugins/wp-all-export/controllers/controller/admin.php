...
abstract class PMXE_Controller_Admin extends PMXE_Controller {
	...
	public function __construct() {		
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- read-only inspection of query string keys to build baseUrl
		$remove = array_diff(array_keys($_GET), $this->baseUrlParamNames);
		if ($remove) {
			$this->baseUrl = esc_url_raw(remove_query_arg($remove));
		} else {
			$this->baseUrl = isset( $_SERVER['REQUEST_URI'] ) ? esc_url_raw( wp_unslash( $_SERVER['REQUEST_URI'] ) ) : '';
		}
		parent::__construct();
        ...
        wp_enqueue_script('jquery-ui-datepicker', PMXE_ROOT_URL . '/static/js/jquery/ui.datepicker.js', array('jquery-ui-core'), PMXE_VERSION, true);
		wp_enqueue_script('tipsy', PMXE_ROOT_URL . '/static/js/jquery/jquery.tipsy.js', array('jquery'), PMXE_VERSION.PMXE_ASSETS_VERSION, true);
		wp_enqueue_script('jquery-pmxe-nestable', PMXE_ROOT_URL . '/static/js/jquery/jquery.mjs.pmxe_nestedSortable.js', array('jquery', 'jquery-ui-dialog', 'jquery-ui-sortable', 'jquery-ui-draggable', 'jquery-ui-droppable', 'jquery-ui-tabs', 'jquery-ui-progressbar'), PMXE_VERSION, true);
		wp_enqueue_script('jquery-select2', PMXE_ROOT_URL . '/static/js/jquery/select2.min.js', array('jquery'), PMXE_VERSION, true);
		wp_enqueue_script('jquery-ddslick', PMXE_ROOT_URL . '/static/js/jquery/jquery.ddslick.min.js', array('jquery'), PMXE_VERSION, true);
		wp_enqueue_script('jquery-chosen', PMXE_ROOT_URL . '/static/js/jquery/chosen.jquery.js', array('jquery'), PMXE_VERSION, true);

        add_action("admin_enqueue_scripts", [$this, 'add_admin_scripts']);

		wp_enqueue_script('jquery-timepicker', PMXE_ROOT_URL . '/static/js/jquery/jquery.timepicker.js', array('jquery'), PMXE_VERSION.PMXE_ASSETS_VERSION, true);

		wp_enqueue_script('wp-pointer');

		/* load plupload scripts */
        wp_enqueue_script('pmxe-admin-script', PMXE_ROOT_URL . '/static/js/admin.js', array('jquery', 'jquery-ui-dialog', 'jquery-ui-datepicker', 'jquery-ui-draggable', 'jquery-ui-droppable', 'jquery-ui-core', 'jquery-ui-widget', 'jquery-ui-position', 'jquery-ui-autocomplete' ), PMXE_VERSION, true);
		wp_enqueue_script('pmxe-scheduling-script', PMXE_ROOT_URL . '/static/js/scheduling.js', array('pmxe-admin-script'), PMXE_VERSION, true);
		wp_enqueue_script('pmxe-admin-validate-braces', PMXE_ROOT_URL . '/static/js/validate-braces.js', array('pmxe-admin-script' ), PMXE_VERSION.PMXE_ASSETS_VERSION, true);
		wp_enqueue_script('pmxe-admin-update-notice', PMXE_ROOT_URL . '/static/js/upgrade-notice.js', array('pmxe-admin-script' ), PMXE_VERSION.PMXE_ASSETS_VERSION, true);

		if(getenv('WPAE_DEV')) {
			// Loaded in the header (not the footer): the admin_head inline script registers the
			// GoogleMerchants 'NONCE' Angular constant and requires this module to already exist.
