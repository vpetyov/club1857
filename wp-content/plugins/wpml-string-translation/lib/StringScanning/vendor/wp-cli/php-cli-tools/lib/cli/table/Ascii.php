	...
	public function row( array $row ) {
		...
		if ( count( $row ) > 0 ) {
			...
			foreach ( $row as $col => $value ) {
				...
				$encoding           = function_exists( 'mb_detect_encoding' ) ? mb_detect_encoding( $value, null, true   ) : false;
				...
				if ( $col_width && ( $original_val_width > $col_width || strpos( $value, "\n" ) !== false ) ) {
					...
					foreach ( $split_lines as $line ) {
						do {
							$wrapped_value = \cli\safe_substr( $line, 0, $col_width, true  , $encoding );
							$val_width     = Colors::width( $wrapped_value, self::isPreColorized( $col ), $encoding );
							if ( $val_width ) {
								$wrapped_lines[] = $wrapped_value;
								$line            = \cli\safe_substr( $line, \cli\safe_strlen( $wrapped_value, $encoding ), null  , false  , $encoding );
							}
						} while ( $line );
					}
		...
		if ( $extra_row_count ) {
			...
			do {
				...
				array_unshift($row_values, '');
				array_push($row_values, '');
