...
function porter_pub_theme_color( $color ) {
	$cmsmasters_option = porter_pub_get_global_options();

	$colors = array(
		'color' => $cmsmasters_option['porter-pub_default_color'],
		'link' => $cmsmasters_option['porter-pub_default_link'],
		'hover' => $cmsmasters_option['porter-pub_default_hover'],
		'heading' => $cmsmasters_option['porter-pub_default_heading'],
		'bg' => $cmsmasters_option['porter-pub_default_bg'],
		'alternate' => $cmsmasters_option['porter-pub_default_alternate'],
		'border' => $cmsmasters_option['porter-pub_default_border'],
		'secondary' => $cmsmasters_option['porter-pub_default_secondary'],
	);
...
function porter_pub_hex_color( $color ) {
	$old_color = porter_pub_theme_color( $color );
...
function porter_pub_tribe_update_default_options() {
	...
	$new_global_elements = array(
		'event_title_color' => porter_pub_hex_color( 'heading' ),
		'event_date_time_color' => porter_pub_hex_color( 'color' ),
		'link_color' => porter_pub_hex_color( 'link' ),
		'background_color_choice' => 'transparent',
		'background_color' => 'transparent',
		'accent_color' => porter_pub_hex_color( 'link' ),
		'map_pin' => '',
	);
	...
	$new_single_event = array(
		...
		'post_title_color' => porter_pub_hex_color( 'heading' ),
	...
	$new_tec_events_bar = array(
		'events_bar_text_color' => porter_pub_hex_color( 'heading' ),
		'find_events_button_text_color' => porter_pub_hex_color( 'bg' ),
		'events_bar_icon_color_choice' => 'default',
		'events_bar_icon_color' => porter_pub_hex_color( 'heading' ),
		'find_events_button_color_choice' => 'custom',
		'find_events_button_color' => porter_pub_hex_color( 'link' ),
		'events_bar_background_color_choice' => 'default',
		'events_bar_background_color' => porter_pub_hex_color( 'bg' ),
		'view_selector_background_color_choice' => 'default',
		'view_selector_background_color' => porter_pub_hex_color( 'bg' ),
		'events_bar_border_color_choice' => 'default',
		'events_bar_border_color' => porter_pub_hex_color( 'border' ),
	);
	...
	$new_month_view = array(
		'days_of_week_color' => porter_pub_hex_color( 'heading' ),
		'date_marker_color' => porter_pub_hex_color( 'heading' ),
		'multiday_event_bar_color_choice' => 'default',
		'multiday_event_bar_color' => '#334aff',
		'grid_lines_color' => porter_pub_hex_color( 'border' ),
		'grid_hover_color' => porter_pub_hex_color( 'border' ),
		'grid_background_color_choice' => 'transparent',
		'grid_background_color' => porter_pub_hex_color( 'color' ),
...
add_action( 'init', 'porter_pub_tribe_update_default_options' );


/* The Events Calendar and related plugins: Add your own location for template file loading. */
function porter_pub_tribe_additional_template_locations( string $file, string $template ) {
	$new_locations = array();

	if ( ! tribe_events_views_v2_is_enabled() ) {
		$new_locations['themes_root'] = trailingslashit( get_theme_root() ) . 'porter-pub/tribe-events';
	}
...
add_filter( 'tribe_events_template', 'porter_pub_tribe_additional_template_locations', 10, 2 );
...
function porter_pub_tribe_events_register_styles_scripts() {
	// Styles
	if ( tribe_events_views_v2_is_enabled() ) {
		wp_enqueue_style( 'porter-pub-tribe-events-v2-style', get_template_directory_uri() . '/tribe-events/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/css/plugin-v2-style.css', array(), '1.0.0', 'screen' );
	} else {
		wp_enqueue_style( 'porter-pub-tribe-events-style', get_template_directory_uri() . '/tribe-events/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/css/plugin-style.css', array(), '1.0.0', 'screen' );

		wp_enqueue_style( 'porter-pub-tribe-events-adaptive', get_template_directory_uri() . '/tribe-events/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/css/plugin-adaptive.css', array(), '1.0.0', 'screen' );

		if ( is_rtl() ) {
			wp_enqueue_style( 'porter-pub-tribe-events-rtl', get_template_directory_uri() . '/tribe-events/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/css/plugin-rtl.css', array(), '1.0.0', 'screen' );
		}
	}
}

add_action('wp_enqueue_scripts', 'porter_pub_tribe_events_register_styles_scripts');


/* Replace Styles */
function porter_pub_tribe_events_stylesheet_url() {
	if ( ! tribe_events_views_v2_is_enabled() ) {
...
add_action('wp_enqueue_scripts', 'porter_pub_tribe_events_stylesheet_url', 100);


/* Replace Pro Styles */
function porter_pub_tribe_events_pro_stylesheet_url() {
	if ( ! tribe_events_views_v2_is_enabled() ) {
...
add_action('wp_enqueue_scripts', 'porter_pub_tribe_events_pro_stylesheet_url', 100);


/* Replace Widget Styles */
function porter_pub_tribe_events_pro_widget_calendar_stylesheet_url() {
	if ( ! tribe_events_views_v2_is_enabled() ) {
		return '';
	}
}

add_filter('tribe_events_pro_widget_calendar_stylesheet_url', 'porter_pub_tribe_events_pro_widget_calendar_stylesheet_url');


/* Replace Responsive Styles */
function porter_pub_tribe_events_mobile_breakpoint() {
	if ( ! tribe_events_views_v2_is_enabled() ) {
		return 768;
	}
}

add_filter('tribe_events_mobile_breakpoint', 'porter_pub_tribe_events_mobile_breakpoint');


/* Events Archive, Venue and Organizer Layout */
function porter_pub_tribe_events_layout($cmsmasters_layout) {
	if (
...
add_filter('cmsmasters_theme_page_layout_filter', 'porter_pub_tribe_events_layout');
