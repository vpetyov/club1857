...
function porter_pub_project_heading($cmsmasters_id, $tag = 'h1', $link_redirect = false, $link_url = false, $link_target = false, $show = true) { 
...
function porter_pub_project_title_nolink($cmsmasters_id, $tag = 'h1', $show = true) { 
...
function porter_pub_project_exc_cont($show = true) {
	$out = cmsmasters_divpdel('<div class="cmsmasters_project_content entry-content">' . "\n" . 
		wpautop(porter_pub_excerpt(20, false)) . 
	'</div>' . "\n");
...
function porter_pub_project_check_exc_cont() {
	$exc = porter_pub_project_exc_cont(false);
...
function porter_pub_get_project_category($cmsmasters_id, $taxonomy, $template_type = 'page', $show = true) {
	$out = '';
	...
	if (get_the_terms($cmsmasters_id, $taxonomy)) {
		if ($template_type == 'page') {
			$out = '<span class="cmsmasters_project_category">' . 
				porter_pub_get_the_category_list($cmsmasters_id, $taxonomy, ', ') . 
			'</span>';
		} elseif ($template_type == 'post') {
			$cmsmasters_option = porter_pub_get_global_options();
			
			
			if ($cmsmasters_option['porter-pub' . '_portfolio_project_cat']) {
				$out .= '<div class="project_details_item">' . 
					'<div class="project_details_item_title">' . esc_html__('Categories', 'porter-pub') . ':' . '</div>' . 
...
function porter_pub_get_project_likes($template_type = 'page', $show = true) {
	$out = '';
	...
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['porter-pub' . '_portfolio_project_like']) {
			$out = '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Likes', 'porter-pub') . ':' . '</div>' . 
...
function porter_pub_get_project_comments($template_type = 'page', $show = true) {
	$out = '';
	
	
	if (comments_open()) {
		if ($template_type == 'page') {
			$out = porter_pub_get_comments('cmsmasters_project_comments');
		} elseif ($template_type == 'post') {
			$cmsmasters_option = porter_pub_get_global_options();
			
			if ($cmsmasters_option['porter-pub' . '_portfolio_project_comment'] && comments_open()) {
				$out = '<div class="project_details_item">' . 
					'<div class="project_details_item_title">' . esc_html__('Comments', 'porter-pub') . ':' . '</div>' . 
					'<div class="project_details_item_desc details_item_desc_comments">' . 
						porter_pub_get_comments('cmsmasters_project_comments') . 
					'</div>' . 
				'</div>';
			}
		}
...
function porter_pub_get_project_date($template_type = 'page', $show = true) {
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['porter-pub' . '_portfolio_project_date']) {
			$out .= '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Date', 'porter-pub') . ':' . '</div>' . 
				'<div class="project_details_item_desc">' . 
...
function porter_pub_get_project_author($template_type = 'page', $show = true) {
	if ($template_type == 'page') {
		$out = '<span class="cmsmasters_project_author">' . 
			esc_html__('By', 'porter-pub') . ' ' . 
			'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Projects by', 'porter-pub') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author" rel="author"><span class="fn">' . esc_html(get_the_author_meta('display_name')) . '</span></a>' . 
		'</span>';
	} elseif ($template_type == 'post') {
		$cmsmasters_option = porter_pub_get_global_options();
		...
		if ($cmsmasters_option['porter-pub' . '_portfolio_project_author']) {
			$out .= '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Author', 'porter-pub') . ':' . '</div>' . 
				'<div class="project_details_item_desc">' . 
					'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Projects by', 'porter-pub') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author" rel="author"><span class="fn">' . esc_html(get_the_author_meta('display_name')) . '</span></a>' . 
				'</div>' . 
			'</div>';
...
function porter_pub_get_project_tags($cmsmasters_id, $taxonomy, $show = true) {
	if (get_the_terms($cmsmasters_id, $taxonomy)) {
		$cmsmasters_option = porter_pub_get_global_options();
		$out = '';
		
		if ($cmsmasters_option['porter-pub' . '_portfolio_project_tag']) {
			$out = '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Tags', 'porter-pub') . ':' . '</div>' . 
				'<div class="project_details_item_desc">' . 
...
function porter_pub_get_project_features($features_position = 'features', $features = '', $features_title = false, $tag = 'h2', $show = true) {
	if (
		(
			!empty($features[0][0]) || 
			!empty($features[0][1])
...
function porter_pub_project_link($link_text, $link_url, $link_target, $show = true) {
	$cmsmasters_option = porter_pub_get_global_options();
	$out = '';
	...
	if ( 
		...
		$out = '<div class="project_details_item">' . 
			'<div class="project_details_item_title">' . esc_html__('Project Link', 'porter-pub') . ':' . '</div>' . 
			'<div class="project_details_item_desc">' . 
